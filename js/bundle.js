...
請嚴格遵守以下格式回傳完整的 JSON 數據，不得包含任何 Markdown 外框（如不可包含 \`\`\`json）：
{
  ...
  function initMap(mapElementId = 'map') {
    ...
    // Standard OpenStreetMap Tiles for 100% reliability
    ...
    // Auto fix container dimensions after DOM render
    setTimeout(() => {
  ...
  class MainApp {
    ...
    renderDashboard() {
      ...
      if (this.panelBudget) renderBudget(this.currentTrip, this.panelBudget);
